...
- `npm run dev`でデバイス一覧を確認し、正しいデバイスIDを使用してください
...
- 使用するモデル（small/large）
- オーディオデバイスのID（推奨）またはインデックス
- 認識時間の調整
- 出力フォーマットの変更

`Vosk.getDevices()`が返す各要素の`id`を`deviceId`に指定すると、デバイスの列挙順に依存せず選択できます。`deviceId`と`deviceIndex`は同時に指定できません。
